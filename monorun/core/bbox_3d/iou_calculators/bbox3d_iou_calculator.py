...
def bbox3d_overlaps(boxes, qboxes, criterion=-1, z_axis=1, z_center=1.0):
    """
    Args:
...
def bbox3d_overlaps_aligned(boxes, qboxes, criterion=-1, z_axis=1, z_center=1.0):
    """
    Args:
        boxes (ndarray): (N, 7), locations [x, y, z], dimensions [l, h, w],
            rot_y
...
def bbox3d_overlaps_aligned_torch(
        boxes, qboxes, criterion=-1, z_axis=1, z_center=1.0):
    """
    Args:
...
def dimonly_iound_aligned_torch(dim_1, dim_2):
    vol_1 = torch.prod(dim_1, dim=1)  # [N]
    vol_2 = torch.prod(dim_2, dim=1)  # [N]
    dim_inter = torch.min(dim_1, dim_2)  # [N, k]
    vol_inter = torch.prod(dim_inter, dim=1)  # [N]
    vol_union = vol_1 + vol_2 - vol_inter  # [N]
